...
.header {text-align:center; margin-bottom:10px; font-size:3em; font-weight:900; color:#0b3d0b;}
.subtitle {text-align:center; margin-bottom:30px; font-size:1.2em; color:#0b3d0b;}
...
if uploaded_file:
    ...
    # Determine predicted class robustly
    if prediction.ndim == 2 and prediction.shape[1] == 1:  # binary sigmoid output
        pred_value = float(prediction[0][0])
        predicted_class = class_names[int(pred_value > 0.5)]
        confidence = pred_value if predicted_class == "Pest" else 1 - pred_value
    elif prediction.ndim == 1:  # just a 1D array
